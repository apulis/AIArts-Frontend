...
import { getEdgeInferences, getFD, submitFD, push, deleteEG } from './service';
...
const EdgeInference = (props) => {
  ...
  const onPush = async (id) => {
    ...
    if (info) {
      ...
      if (code === 0) {
        getData();
        message.success(formatMessage({ id: 'edgeInference.list.onPush.success' }));
      } else {
        setPushId('');
      }
    } else {
      message.warning(formatMessage({ id: 'edgeInference.list.onPush.error' }));
      setModalFlag2(true);
    }
  };
  ...
  const onDelete = (id) => {
    confirm({
      title: formatMessage({ id: 'edgeInference.list.onDelete.title' }),
      icon: <ExclamationCircleOutlined />,
      okText: formatMessage({ id: 'edgeInference.list.onDelete.okText' }),
      okType: 'danger',
      cancelText: formatMessage({ id: 'edgeInference.list.onDelete.cancelText' }),
      onOk: async () => {
        ...
        if (code === 0) {
          ...
          message.success(formatMessage({ id: 'edgeInference.list.onDelete.ok.tips' }));
        }
      },
      onCancel() {},
    });
  ...
  const columns = [
    {
      title: formatMessage({ id: 'edgeInferenceList.table.column.id' }),
      dataIndex: 'jobId',
      render: (id) => <span style={{ fontFamily: 'Consolas' }}>{id}</span>,
    },
    {
      title: formatMessage({ id: 'edgeInferenceList.table.column.name' }),
      dataIndex: 'jobName',
      key: 'jobName',
      sorter: true,
    ...
    {
      title: formatMessage({ id: 'edgeInferenceList.table.column.type' }),
      dataIndex: 'modelconversionType',
    },
    {
      title: formatMessage({ id: 'edgeInferenceList.table.column.time' }),
      dataIndex: 'jobTime',
      key: 'jobTime',
      sorter: true,
    ...
    {
      title: formatMessage({ id: 'edgeInferenceList.table.column.status' }),
      render: (item) => {
        ...
        if (modelconversionStatus === 'converting')
          status =
            jobStatus === 'finished'
              ? formatMessage({ id: 'edgeInference.list.converting.result.success' })
              : jobStatus === 'failed' || jobStatus === 'error'
              ? formatMessage({ id: 'edgeInference.list.converting.result.error' })
              : status;
        return <span>{status}</span>;
      },
    },
    {
      title: formatMessage({ id: 'edgeInferenceList.table.column.action' }),
      render: (item) => {
        ...
        return (
          <>
            <a onClick={() => onPush(jobId)} disabled={disabled}>
              {formatMessage({ id: 'edgeInferenceList.table.column.action.push' })}
            </a>
            <a style={{ color: 'red', marginLeft: 16 }} onClick={() => onDelete(jobId)}>
              {formatMessage({ id: 'edgeInferenceList.table.column.action.delete' })}
            </a>
          </>
        );
      },
  ...
  const onSubmitFD = () => {
    ...
    form.validateFields().then(async (values) => {
      ...
      if (code === 0) {
        message.success(formatMessage({ id: 'edgeInference.list.onSubmitFD.success' }));
        getFdInfo();
        setModalFlag2(false);
      }
    });
  ...
  const getOptions = () => {
    const statusMap = [
      {
        text: formatMessage({ id: 'edgeInference.list.statusMap.all' }),
        status: '',
      },
      {
        text: formatMessage({ id: 'edgeInference.list.statusMap.push.ing' }),
        status: 'finished-pushing',
      },
      {
        text: formatMessage({ id: 'edgeInference.list.statusMap.push.success' }),
        status: 'finished-push success',
      },
      {
        text: formatMessage({ id: 'edgeInference.list.statusMap.push.error' }),
        status: 'finished-push failed',
      },
      {
        text: formatMessage({ id: 'edgeInference.list.statusMap.converting.ing' }),
        status: 'running,scheduling,queued,unapproved-converting',
      },
      {
        text: formatMessage({ id: 'edgeInference.list.statusMap.converting.success' }),
        status: 'finished-converting',
      },
      {
        text: formatMessage({ id: 'edgeInference.list.statusMap.converting.error' }),
  ...
  return (
    <PageHeaderWrapper>
      <Card>
        <div className={styles.edgeInferences}>
          <Link to="/Inference/EdgeInference/submit">
            <Button type="primary">
              {formatMessage({ id: 'edgeInference.list.add.inference' })}
            </Button>
          </Link>
          <Button type="primary" style={{ margin: '0 16px 16px' }} onClick={openSettings}>
            {formatMessage({ id: 'edgeInference.list.setting' })}
          </Button>
          {fdInfo.url && (
            <Button type="primary" onClick={() => window.open(fdInfo.url)}>
              {formatMessage({ id: 'edgeInference.list.fdServer' })}
            </Button>
          )}
          <div className={styles.searchWrap}>
            ...
            <Search
              placeholder={formatMessage({ id: 'edgeInference.list.placeholder.search' })}
              enterButton
            ...
            <Button
              onClick={() =>
                getData(formatMessage({ id: 'edgeInference.list.fresh.success' }))
              }
              icon={<SyncOutlined />}
            />
          </div>
      ...
      {modalFlag2 && (
        <Modal
          title={formatMessage({ id: 'edgeInference.list.setting' })}
          visible={modalFlag2}
          ...
          footer={[
            <Button onClick={() => setModalFlag2(false)}>
              {formatMessage({ id: 'edgeInference.list.cancel' })}
            </Button>,
            <Button type="primary" loading={btnLoading} onClick={onSubmitFD}>
              {formatMessage({ id: 'edgeInference.list.save' })}
            </Button>,
          ]}
        >
          <Form form={form} initialValues={fdInfo}>
            <Form.Item
              label={formatMessage({ id: 'confirmEdgeInferencePush.label.url' })}
              name="url"
              rules={[
                {
                  required: true,
                  message: formatMessage({ id: 'confirmEdgeInferencePush.rule.needUrl' }),
                },
              ]}
            >
              <Input
            ...
            <Form.Item
              label={formatMessage({ id: 'confirmEdgeInferencePush.label.username' })}
              name="username"
              rules={[
                {
                  required: true,
                  message: formatMessage({ id: 'confirmEdgeInferencePush.rule.needUsername' }),
                },
              ]}
            >
              <Input
            ...
            <Form.Item
              label={formatMessage({ id: 'confirmEdgeInferencePush.label.password' })}
              name="password"
              rules={[
                {
                  required: true,
                  message: formatMessage({ id: 'confirmEdgeInferencePush.rule.needPassword' }),
                },
              ]}
            >
              <Input
