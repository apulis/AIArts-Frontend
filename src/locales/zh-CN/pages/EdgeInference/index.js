export default {
  'edgeInference.list.add.inference': '创建边缘推理',
  'edgeInference.list.setting': '设置',
  'edgeInference.list.placeholder.search': '输入名称搜索',
  'edgeInference.list.typeText.converting': '转换中',
  'edgeInference.list.typeText.push': '推送中',
  'edgeInference.list.typeText.push.success': '推送成功',
  'edgeInference.list.typeText.push.failed': '推送失败',

  'edgeInference.list.onDelete.title': '确定要删除该推理吗？',
  'edgeInference.list.onDelete.okText': '删除',
  'edgeInference.list.onDelete.cancelText': '取消',
  'edgeInference.list.onDelete.ok.tips': '删除成功！',

  'edgeInference.list.converting.result.success': '转换成功',
  'edgeInference.list.converting.result.error': '转换失败',

  'edgeInference.list.onPush.success': '推送成功！',
  'edgeInference.list.onPush.error': '请先填写设置！',

  'edgeInference.list.onSubmitFD.success': '设置成功！',

  'edgeInference.list.statusMap.all': '全部',
  'edgeInference.list.statusMap.push.ing': '推送中',
  'edgeInference.list.statusMap.push.success': '推送成功',
  'edgeInference.list.statusMap.push.error': '错误',
  'edgeInference.list.statusMap.converting.ing': '转换中',
  'edgeInference.list.statusMap.converting.success': '转换成功',
  'edgeInference.list.statusMap.converting.error': '转换错误',

  'edgeInference.list.fdServer': 'FD服务器',
  'edgeInference.list.fresh.success': '刷新成功！',
  'edgeInference.list.setting': '设置',
  'edgeInference.list.cancel': '取消',
  'edgeInference.list.save': '保存',

  'edgeInference.create.submit.success': '提交成功',
  'edgeInference.create.title': '提交边缘推理',
  'edgeInference.create.clickForAddParams': '点击增加参数',
};
